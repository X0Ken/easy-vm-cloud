...
export class WebSocketService {
  ...
  // 连接状态
  private connectionStateSubject = new BehaviorSubject<'disconnected' | 'connecting' | 'connected'>(
    'disconnected',
  );
  public connectionState$ = this.connectionStateSubject.asObservable();

  // VM 状态更新流
  private vmStatusUpdateSubject = new Subject<{
    vm_id: string;
    status: string;
    message?: string;
  }>();
  ...
  // 节点状态更新流
  private nodeStatusUpdateSubject = new Subject<{
    node_id: string;
    status: string;
    message?: string;
  }>();
  ...
  // 快照状态更新流
  private snapshotStatusUpdateSubject = new Subject<{
    snapshot_id: string;
    status: string;
    message?: string;
  }>();
  public snapshotStatusUpdates$ = this.snapshotStatusUpdateSubject.asObservable();

  // 任务状态更新流
  private taskStatusUpdateSubject = new Subject<{
    task_id: string;
    status: string;
    progress?: number;
    message?: string;
  ...
  // 系统通知流
  private systemNotificationSubject = new Subject<{
    title: string;
    message: string;
    level: string;
  }>();
  ...
  private handleMessage(event: MessageEvent): void {
    try {
      ...
      switch (message.type) {
        ...
        case 'SnapshotStatusUpdate':
          if (message.snapshot_id && message.status) {
            this.snapshotStatusUpdateSubject.next({
